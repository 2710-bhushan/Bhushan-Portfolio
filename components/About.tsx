      ...
      <div className="about-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'start' }}>
        ...
        <div className="about-stats-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {[
            ...
            { num: '10+', label: 'Projects Built', color: 'var(--accent3)', icon: FaLaptopCode },
            { num: 'MERN', label: 'Stack Focus', color: 'var(--gold)', icon: FaScrewdriverWrench },
          ].map((s: { num: string; label: string; color: string; icon: IconType }, i) => (
